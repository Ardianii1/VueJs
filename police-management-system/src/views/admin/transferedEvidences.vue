            ...
            <div class="bg-white dark:bg-gray-800 mb-3 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-hidden ">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 overflow-x-hidden">
                        ...
                        <tbody class="">
                            <tr v-for="evidences in itemsToDisplay" :key="evidences.id"
                                class="border-b dark:border-gray-700 overflow-x-scroll">
                                <td class="px-4 py-3 dark:text-white truncate">{{ evidences.transferDate }}</td>
                                <td class="px-4 py-3 truncate">{{ evidences.transferedFrom }}</td>
                                <td class="px-4 py-3 truncate">{{ evidences.transferedTo }}</td>
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    ...
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">1-{{ evidences.length < pageSize ?
                            evidences.length : pageSize }}</span>
                                of
...
export default {
    ...
    methods: {
        async handleDeleteEvidence(id) {
            ...
            let apiURL = `http://localhost:3000/evidences/deletetransfered/transfered/${id}`;
    ...
    computed: {
        ...
        filteredEvidences() {
            return this.evidences.filter((evidence) => {
                const fields = ['transferDate', 'transferedFrom', 'transferedTo', 'caseNumber']; // replace with actual fields
    ...
    created() {
        let apiURL = `http://localhost:3000/evidences/transfered`;
