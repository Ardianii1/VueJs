    ...
    <div class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
        <div class="md:ml-64 h-screen  px-4 lg:px-12">
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <h3 class="text-white text-center py-3 font-bold tracking-widest">Accidents</h3>
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    ...
                    <div
                        ...
                        <router-link to="/reports/accidentReport">
                            <button type="button"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="accidents in itemsToDisplay" :key="accidents.id" class="border-b dark:border-gray-700">
                                <!-- <td class="px-4 py-3 dark:text-white truncate">{{ accidents.Type }}</td> -->
                                <td class="px-4 py-3 truncate">{{ accidents.Locationn }}</td>
                                <td class="px-4 py-3 truncate">{{ accidents.Dateof }}</td>
                                <td class="px-4 py-3 truncate">{{ accidents.CreatedAt }}</td>
                                <td class="px-4 py-3 truncate text-right ">
                                    <router-link :to="`/accidents/details/${accidents._id}`"
                                    ...
                                    <button type="button" @click="handleDeleteCrime(accidents._id)"
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    ...
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">1-{{ accidents.length > 7 ? 7 :
                            accidents.length
                        }}</span>
                        of
                        <span class="font-semibold text-gray-900 dark:text-white">{{ accidents.length }}</span>
                    </span>
...
export default {
    data() {
        return {
            allaccidents: this.$store.dispatch("fetchAccidents"),
            sortBy: 'casenumber',
            sortDirection: 1,
            searchKeyword: '',
            pageSize: 7,
    ...
    methods: {
        handleDeleteCrime(id) {
            let apiURL = `http://localhost:3000/accidents/delete/${id}`;
            let indexOfArrayItem = this.accidents.findIndex((i) => i._id === id);
            if (window.confirm("Do you really want to delete?")) {
                axios
                    .put(apiURL)
                    .then(() => {
                        this.accidents.splice(indexOfArrayItem, 1);
                    })
                    .catch((error) => {
                        console.log(error);
                    });
    ...
    computed: {
        ...
        filteredAccidents() {
            return this.accidents
                .filter((accidents) => {
                    // console.log(casee)
                    const fields = ['Dateof', 'Locationn', 'Type', 'CreatedAt']; // replace with actual fields
                    for (const field of fields) {
                        if (accidents[field].toLowerCase().includes(this.searchKeyword.toLowerCase())) {
                            return true;
                        }
                    }
                    return false;
        ...
        sortedAccidents() {
            return this.accidents.sort((a, b) => {
                if (a[this.sortBy] > b[this.sortBy]) return 1 * this.sortDirection;
                if (a[this.sortBy] < b[this.sortBy]) return -1 * this.sortDirection;
                return 0;
            });
        ...
        totalPages() {
            return Math.ceil(this.accidents.length / this.pageSize);
        },
        pagesToShow() {
        ...
        itemsToDisplay() {
            ...
            return this.filteredAccidents.slice(startIndex, endIndex);
        }
    },

    created() {
        this.$store.dispatch("fetchAccidents");
    },
};
</script>
