            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    ...
                    <div
                        ...
                        <router-link to="/reports/crimeReport">
                            <button type="button"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <tr v-for="crimes in itemsToDisplay" :key="crimes.id" class="border-b dark:border-gray-700">
                                <td class="px-4 py-3 dark:text-white truncate">{{ crimes.Type }}</td>
                                <td class="px-4 py-3 truncate">{{ crimes.Locationn }}</td>
                                ...
                                <td class="px-4 py-3 truncate text-right ">
                                    ...
                                    <button type="button" @click="handleDeleteCrime(crimes._id)"
                ...
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
                    ...
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing
                        <span class="font-semibold text-gray-900 dark:text-white">1-{{ crimes.length > 7 ? 7 :
                            crimes.length
                        }}</span>
                        of
                        <span class="font-semibold text-gray-900 dark:text-white">{{ crimes.length }}</span>
                    </span>
...
export default {
    data() {
        return {
            allcrimes: this.$store.dispatch("fetchCrimes"),
            sortBy: 'casenumber',
            sortDirection: 1,
            searchKeyword: '',
            pageSize: 7,
    ...
    methods: {
        handleDeleteCrime(id) {
            let apiURL = `http://localhost:3000/crimes/delete/${id}`;
            let indexOfArrayItem = this.crimes.findIndex((i) => i._id === id);
            if (window.confirm("Do you really want to delete?")) {
                axios
                    .put(apiURL)
                    .then(() => {
                        this.crimes.splice(indexOfArrayItem, 1);
                    })
                    .catch((error) => {
                        console.log(error);
                    });
    ...
    computed: {
            ...
            return this.crimes
                .filter((crimes) => {
                    ...
                    for (const field of fields) {
                        if (crimes[field].toLowerCase().includes(this.searchKeyword.toLowerCase())) {
                            return true;
                        }
                    }
                    return false;
        ...
        totalPages() {
            return Math.ceil(this.crimes.length / this.pageSize);
        },
        pagesToShow() {
        ...
        itemsToDisplay() {
            ...
            return this.filteredCrimes.slice(startIndex, endIndex);
        }
    },

    created() {
        this.$store.dispatch("fetchCrimes");
    },
};
</script>
